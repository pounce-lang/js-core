import { split, head, tail, is, append, findLast, complement, isNil, prop, prepend } from 'ramda';
...
var pounce = function (pl, stack, wordstack) {
    if (pl.length > 0) {
        ...
        var restPl = tail(pl);
        if (is(Array, term)) {
            return pounce(restPl, append(term, stack), wordstack);
        }
        else if (is(String, term)) {
            var termStr = term.toString();
            console.log(term);
            var thisWord = findLast(hasWord(termStr))(wordstack);
            console.log('*** thisWord', thisWord);
            var thisWD = prop(termStr);
            console.log('*** thisWD', thisWD(thisWord));
            if (thisWord && complement(isNil)(thisWD(thisWord))) {
